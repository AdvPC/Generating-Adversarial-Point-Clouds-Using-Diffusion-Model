...
from utils_v2.model_utils import calc_dcd
from .common import *
...
class DiffusionPoint(Module):
    ...
    def sample1(self, x, context, flexibility=0.0, ret_traj=False):
        ...
        for t in range(self.var_sched.num_steps, 80, -1):
            noise_scale = 0.05 * t / self.var_sched.num_steps  # 根据T调整噪音大小
            # z = torch.randn_like(x_T) if t > 1 else torch.zeros_like(x_T)
            z = torch.randn_like(x_T) * noise_scale if t > 1 else torch.zeros_like(x_T)
            ...
            x_next = (c0 * (x_t - c1 *e_theta)) + sigma * x_T
            ...
            # 保存前十次生成的点云
            if t >= 80 - 10:
                saved_points.append(traj[t].numpy())  # 保存当前轨迹点
            ...
            # 保存前十次生成的点云到npy文件
        if saved_points:
            saved_points = np.stack(saved_points)
            np.save('saved_points.npy', saved_points)
